<form method="post" action="<?= base_url('admin/settings') ?>" enctype="multipart/form-data">
  ...
  <div class="card">
    <div class="card-header"><h3><?= __('settings.ads_section') ?></h3></div>
    ...
    <?php if ($advertisements): ?>
      <div class="grid grid-cols-4 mb-4">
        <?php foreach ($advertisements as $ad): ?>
          <div class="card" style="padding:10px;">
            <img src="<?= upload_url($ad['image_path']) ?>" class="thumb-sm mb-2" style="width:100%;height:90px;object-fit:contain;background:var(--color-slate-light);" alt="">
            ...
            <button type="submit" form="ads-delete-<?= (int) $ad['id'] ?>" class="btn btn-danger btn-sm" style="width:100%;"><?= __('common.delete') ?></button>
          </div>
        <?php endforeach; ?>
      </div>
    <?php endif; ?>
    ...
    <div class="form-row" style="align-items:flex-end;">
      <div class="form-group">
        ...
        <input type="text" name="business_name" form="ads-add-form" class="form-control" required>
      </div>
      <div class="form-group">
        <label><?= __('settings.ads_link_url') ?></label>
        <input type="url" name="link_url" form="ads-add-form" class="form-control" placeholder="https://...">
      </div>
    </div>
    ...
    <div class="form-row" style="align-items:flex-end;">
      <div class="form-group">
        ...
        <input type="file" name="image" form="ads-add-form" class="form-control" accept="image/jpeg,image/png,image/webp" required>
      </div>
      <div class="form-group" style="flex:0 0 auto;">
        <button type="submit" form="ads-add-form" class="btn btn-primary"><?= __('settings.ads_add_button') ?></button>
      </div>
    </div>
...
<?php endforeach; ?>
<form id="ads-add-form" method="post" action="<?= base_url('admin/settings/advertisements') ?>" enctype="multipart/form-data" style="display:none;"><?= csrf_field() ?></form>
<?php foreach ($advertisements as $ad): ?>
  <form id="ads-delete-<?= (int) $ad['id'] ?>" method="post" action="<?= base_url('admin/settings/advertisements/' . $ad['id'] . '/delete') ?>" data-confirm="<?= e(__('zone.delete_confirm')) ?>" style="display:none;"><?= csrf_field() ?></form>
<?php endforeach; ?>
